    ...
    <script type="text/javascript">
        ...
        barcodeReaderCallback = function(data) {
			...
			// 명령어
				...
				// } // 종료 시간 입력 코드 작성
				if (uniqueId == 4) { 
					$("#endtime_" + select_work_id)[0].value = today.toISOString().slice(0, -8)
					} // 등록 버튼 코드 작성
				else if (uniqueId == 5) { 
					$("#update_" + select_work_id)[0].click()
				} // 뒤로 가기 코드
				else if (uniqueId == 6) { 
					history.back();
				}
		...
		<div class="form-inline" class="row">
			<div class="col-12" id="maindiv">
				...
				while(rs.next()){
					...
					if (tempstring.equals("완료") != true && firstNum == -1) { 
						firstNum = datalastdivnum;
					}
				%>

				<div id='<%=datalastdivnum%>' style="<%=tempstring.equals("완료")?"display:none":""%>">
					<form>
					<div id='barcode_<%=datalastdivnum%>'> </div>
						...
						<table class="table">
							...
							<tbody>
								<tr>
									...
									<td>
										<input type="datetime-local" class="form-control" id="endtime_<%=datalastdivnum%>" value="<%=workend%>" name="work_end">
									</td>
									<td>
									...
									<td style="width: 10%">
										<button class="btn btn-info" id="update_<%=datalastdivnum%>" type="submit" formaction="update.jsp" formmethod="post">수정</button>
									</td>
								</tr>
							</tbody>
						</table>
					...
					<%
						String codetest = "19"; 
						String blank = "";
						String dataid = String.valueOf(datalastdivnum);

						for (int ippp = 0; ippp < 12 - dataid.length() - codetest.length(); ippp++) { 
							blank = blank + "0";
						}
						String  datatest = codetest + blank + dataid;
					%>
					drawBarcode('#barcode_<%=datalastdivnum%>', "<%=datatest%>", ratio = 3)
					</script>
				</div>
				<%} %>
				<script>
				var lastdivnum = <%=datalastdivnum%>;
				select_work_id = <%=firstNum%>;
				</script>
			</div>
		</div>
...
<div style="display: flex; justify-content: center;">
	...
	<img src="./resources/55_5_edit.png" style="margin: 10px;">
